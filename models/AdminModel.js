...
getParticipantCountByWeek: async () => {
  ...
  updateAdmin: async (adminId, newData) => {
    try {
        const { email, mots_de_passe } = newData;

        if (!email && !mots_de_passe) {
            throw new Error('Au moins un champ (email ou mots_de_passe) doit être fourni pour la mise à jour.');
        }
        ...
        if (mots_de_passe) {
            const hashedPassword = await bcrypt.hash(mots_de_passe, saltRounds);
            updateFields.push('mots_de_passe = ?');
            queryParams.push(hashedPassword);
        }

        queryParams.push(adminId);

        const updateQuery = `UPDATE admin SET ${updateFields.join(', ')} WHERE id_A = ?`;
